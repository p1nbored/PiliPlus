...
```
E:\project\PiliPlus                             应用本体（分支 ohos）
...
### 首先要确认的一点：hybrid composition 是否可用

这是整套方案唯一没能在本地静态验证的环节，**请优先确认**。

Flutter 框架侧是齐全的：`ExpensiveOhosViewController` 使用
`_HybridOhosViewControllerInternals`，`OhosViewSurface` 在
`requiresViewComposition` 为真时会走 `_PlatformLayerBasedOhosViewSurface`。
但真正把节点挂上去的是引擎侧的 `createForPlatformViewLayer` →
`configureForHybridComposition`。在能拿到源码的旧引擎分支
（openharmony-sig/flutter_engine `br_3.22.0-ohos-1.0.4`）里，
**这个函数是个只打日志的空实现**。本项目用的 3.41.9 引擎是预编译产物
（`bin/internal/engine.ohos.har.version` = `3fb08d34…`），没有对应源码可查，
因此无法确认它是否已经实现。

一个旁证：同样在本项目里的 `flutter_inappwebview_ohos` 用的是
`PlatformViewsService.initOhosView`（纹理合成），代码注释写着
“这里对比安卓没有 PlatformViewLink 暂时用 OhosView 替代”。

**症状与结论**：

- 视频区域黑屏 / 完全不显示，但音频正常、mpv 日志正常
  → 该引擎的 hybrid composition 没有实现。此时纹理路径必然拿不到 HDR，
  修复需要额外打引擎补丁（实现 `configureForHybridComposition`，
  参考同文件里 `configureForTextureLayerComposition` 的写法，
  把 `NodeRenderType` 换成 `RENDER_TYPE_DISPLAY` 并把节点挂进视图树）。
- 视频正常显示且出现 `NativeWindow output switched to BT.2020 PQ`
  → hybrid composition 可用，HDR 链路打通。
